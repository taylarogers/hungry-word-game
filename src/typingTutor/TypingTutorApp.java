...
//shared class variables
	...
	/** 
	This creates and setups up the GUI and its elements
	*/
	public static void setupGUI(int frameX,int frameY,int yLimit) {
		// Create frame and its components
		...
		gameWindow = new GamePanel(words,hungryWord,yLimit,done,started,won); //
	   ...
	   // Actions that occur when enter is pressed
	...
	/** 
	This creates all the threads necessary for the program
	*/
	...
	/** 
	This creates all of the word mover threads needed for the program for the falling words and the hungry word
	*/
	public synchronized static void createWordMoverThreads() {
		score.reset();
		...
		// Set hungry word and assign it to the classes that need it
		hungryWord = new FallingWord(dict.getNewWord(), (int)(yLimit/2), xLimit, true);

		gameWindow.setHungryWord(hungryWord);
		...
		hungryWordShift = new HungryWordMover(hungryWord, words, dict, score, startLatch, done, pause); //
		...
		// Start the hungry word mover
		if (!hungryWordShift.isAlive())
			hungryWordShift.start(); //	
	}
...
public static void main(String[] args) {
		...
		xLimit = frameX-(25*4); // Max x-value
		...
		hungryWordShift = new HungryWordMover(hungryWord);
		
		CatchWord.setWords(words, hungryWord);  //class setter - static method
